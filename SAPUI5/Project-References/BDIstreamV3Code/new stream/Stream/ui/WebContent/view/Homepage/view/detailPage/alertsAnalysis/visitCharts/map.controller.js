sap.ui.controller("sap.rds.bdi.stream.Homepage.view.detailPage.alertsAnalysis.visitCharts.map", {
    markers : [],
    map : null,
    infoBubbles : null,
    drawpoly : "drawPoly",
    ...
    onAfterRendering: function() {
        ...
        this.map = new nokia.maps.map.Display(document.getElementById("visitMapContainerA"), {
            zoomLevel: 5,
            center: myCenter,
            components: [
        ...
        this.toolKit.placeAt("polygonToolA", "only");
    ...
    getMapData: function() {
        ...
        var afterDate = util.dateToString(sap.ui.getCore().byId("alertsDate").getDateValue());
        var beforeDate = util.dateToString(sap.ui.getCore().byId("alertsDate").getSecondDateValue());
        ...
        this.oService = "ui/xsjs/api.xsjs/alert/getAlertCountMap?after=" + afterDate +
            "&before=" + beforeDate + "&serverIP=" + this.serverIP;

        this.oServer = "ui/xsjs/api.xsjs/alert/getAlertServerMap?after=" + afterDate +
            "&before=" + beforeDate + "&serverIP=" + this.serverIP;
    ...
    refreshData : function() {
        ...
        this.oHeader = "ui/xsjs/api.xsjs/intelligence/getAlertCountPoly?after=" + afterDate + "&before=" + beforeDate
        ...
        kpiValue = this.entityHeader.businessData[0].NUMBER_OF_ATTACK_TOTAL;
        sap.ui.getCore().byId("alertsKpiValue").setText(kpiValue);
    },
    ...
    loadMarkers : function() {
        ...
        var markerMouseoverHandler = function (evt) {
            ...
            function generateBusyIndicator() {
                var busyIndicator = document.getElementById("busyIndicatorAV");
                if(!busyIndicator){
                    $("body").append("<div id='busyIndicatorAV'></div>");
                    (new sap.m.BusyIndicator()).placeAt("busyIndicatorAV", "only");
                    busyIndicator = document.getElementById("busyIndicatorAV")
                }
                ...
                this.entityList.businessData.forEach(
                    jQuery.proxy(function(entity) {
                        ...
                        if(city == entity.City) {
                            ...
                            var data = {};
                            data.AlertType = entity.AlertType;
                            data.SpatialCount = entity.SpatialCount;
                            datas.push(data);

                            // If entity has only one element.
                            if(this.entityList.businessData.length == 1 || idx == this.entityList.businessData.length-1) {
        ...
        if(this.entityListServer.serverData){
            ...
            if(this.entityListServer.serverData.length != 0){
                this.entityListServer.serverData.forEach(
                    jQuery.proxy(function(entity) {
                            if (entity.Server_Name!= serverAux) {
                                datas = [];
