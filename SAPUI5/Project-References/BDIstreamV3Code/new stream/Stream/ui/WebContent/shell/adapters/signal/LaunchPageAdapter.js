...
(function () {
    ...
    jQuery.sap.declare("sap.ushell.adapters.signal.LaunchPageAdapter");
    ...
    sap.ushell.adapters.signal.LaunchPageAdapter = function (oSystem) {
        var aConfigGroups = jQuery.extend(true, [], sap.ushell.shells.signal.fioriDemoConfig.groups),
            aConfigCatalogs = sap.ushell.shells.signal.fioriDemoConfig.catalogs,
            ...
            if(bNewVisible){
                ...
                oTile.intervalTimer = setInterval(function(){
                   /* OData.read(
                	...
                	//only refresh at homepage
               	 var refreshFlag = true;
                    var acturalUrl = location.toString(); 
                    var hashPosition = acturalUrl.lastIndexOf("#");
                    if (hashPosition > 0) {
                    	var afterHash = acturalUrl.substring((hashPosition+1));
                    	if (afterHash) {
                    		refreshFlag = false;
                    	}
                    }   
               	if (refreshFlag) {
               		oTile.properties.refreshKPI(oTile);
               	}
        ...
        this.resetGroup = function (oGroup) {
            ...
            window.setTimeout(function () {
                if (!bFail) {
                    ...
                    oGroup = jQuery.extend(true, {}, sap.ushell.shells.signal.fioriDemoConfig.groups[indexOfGroup(sap.ushell.shells.signal.fioriDemoConfig.groups, oGroup)]);
...
//        };
        this.getTileView = function (oTile) {
            ...
            if (oTile.tileType) {
                ...
                if (ViewPrototype) {
                    try {
                    	var tileView = new ViewPrototype(oTile.properties || {});
                    	var model=new sap.ui.model.json.JSONModel();
                    	if (!tileView.getModel()){
                    		tileView.setModel(model);
                    		oTile.properties["model"] = model;
                    	}
                    	if (oTile.properties.refreshKPI){
                    		oTile.properties.refreshKPI(oTile);
                    	}
                        return tileView;
        ...
        this.setTileVisible = function (oTile, bNewVisible) {
        	document.title = "SAP Stream Intelligence";
